...
{/* TODO: Conditional render for the profile thing */}
          <Link to={`/profile/${profile?.address}`} className='navbar__right__user__link'>
            <img
              className='navbar__right__user__userimage'
              src={profile?.profileImage[0].url}
              alt=""
            />
            <span className='navbar__right__user__username'>{profile?.name}</span>
          </Link>
        </div>
